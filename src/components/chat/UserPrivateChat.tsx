...
interface UserPrivateChatProps {
  socket: Socket;
  userItem: UserListPropsType;
}

const UserPrivateChat: React.FC<UserPrivateChatProps> = ({
  socket,
  userItem,
}) => {
  ...
  useEffect(() => {
    async function init() {
      await handleGetPrivateChatHistory(pageCount);
    }
  ...
  useEffect(() => {
    if (!isEmpty(messageData)) {
      handleReceiveMessageToPrivate(messageData);
    }
  }, [messageData]);
    ...
    try {
      if (ev && ev.target) {
        setTimeout(async () => {
          ...
          await handleGetPrivateChatHistory(pageCount + 20);
          ev.target.complete();
  ...
  const handleGetPrivateChatHistory = async (pageCount: number) => {
    try {
      const res = await Http.request({
        method: "POST",
        url: apiURL + "/api/chats/getPrivateChatData",
        headers: {
          "Content-Type": "application/json",
        },
        data: { pageCount, recipient: userItem.id, user: auth.user.id },
      });
      ...
      if (res.status === 200) {
        if (isEmpty(res.data.results)) {
          ...
          let tempMessageList: PrivateChatHistoryResType[] = [];
          setChatHisTotalCount(res.data.totalCount);

          const results = res.data.results.sort(
            (a: PrivateChatHistoryResType, b: PrivateChatHistoryResType) => {
          ...
          setMessageList([
            ...tempMessageList.map((item: PrivateChatHistoryResType) => {
              return {
                id: item.user._id,
  ...
  socket.on("receiveMessageToPrivate", (msgData) => {
    setMessageData(msgData);
  });

  const handleReceiveMessageToPrivate = (msgData: any) => {
  ...
  const handleKeyDown = (event: React.KeyboardEvent<HTMLIonInputElement>) => {
    if (event.key === "Enter") {
      ...
      const msgData = {
        ...
        recipient: userItem.id,
      };

      socket.emit("sendMessageToPrivate", msgData);
  ...
  const handleSaveUnreadMessageCount = async (count: number) => {
    ...
    try {
      const res = await Http.request({
        method: "POST",
        url: apiURL + "/api/chats/savePrivateUnReadMessageCount",
        headers: {
          "Content-Type": "application/json",
        },
        data: paramData,
